...
if ($clone_id > 0) {
    $stmt_base = $pdo->prepare("SELECT * FROM solicitacoes_hae WHERE id = ? AND professor_id = ?");
...
$c_categoria = $registro_base['categoria'] ?? '';
$c_justificativa = $registro_base['justificativa'] ?? '';
$c_objetivo = $registro_base['objetivo'] ?? '';
$c_metodologia = $registro_base['metodologia'] ?? '';
$c_envolvidos = $registro_base['envolvidos'] ?? '';
$c_detalhamento = $registro_base['detalhamento_recursos'] ?? '';
$c_cronograma = $registro_base['cronograma'] ?? '';
$c_resultados = $registro_base['resultados_esperados'] ?? '';
$c_coordenador_alvo = $registro_base['coordenador_alvo_id'] ?? '';

$recursos_array = [];
if ($registro_base && !empty($registro_base['recursos_necessarios'])) {
...
<head>
    ...
    <style>
        ...
        .custom-dropdown { 
            ...
            border: 1px solid #ddd; border-radius: 6px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); 
            max-height: 200px; overflow-y: auto; z-index: 1000; list-style: none; margin-top: 5px;
        }
        .custom-dropdown li { 
            padding: 10px 15px; display: flex; justify-content: space-between; align-items: center; 
            border-bottom: 1px solid #f5f5f5; cursor: pointer; font-size: 14px; color: #444; transition: 0.2s;
        }
        .custom-dropdown li:hover { background-color: #f4fbf7; color: #1e824c; }
        .btn-excluir-cat { color: #ccc; background: none; border: none; cursor: pointer; padding: 5px; font-size: 14px; transition: 0.2s; }
        .btn-excluir-cat:hover { color: #e74c3c; }

        .box-encaminhamento { background-color: #f8f9fa; padding: 20px; border-radius: 8px; border-left: 4px solid #3498db; margin-bottom: 30px; }
...
<body>
    ...
    <main class="main-content">
        ...
        <?php if (!$tem_assinatura): ?>
            <div class="form-card" style="text-align: center; padding: 50px 20px;">
                ...
                <h2 style="color: var(--fatec-red); margin-bottom: 15px;">Assinatura Digital Obrigatória</h2>
                <p style="color: #555; margin-bottom: 25px; font-size: 15px; line-height: 1.6; max-width: 550px; margin-left: auto; margin-right: auto;">
                    Para garantir a validade legal e institucional dos documentos gerados por este sistema, é <strong>obrigatório</strong> que você tenha uma assinatura digital registrada na sua conta antes de criar ou editar um projeto HAE.
                </p>
                <a href="perfil.php" style="background: var(--fatec-red); color: white; padding: 15px 30px; border-radius: 6px; text-decoration: none; font-weight: bold; display: inline-block; font-size: 15px; transition: 0.3s;">
                    <i class="fa-solid fa-arrow-up-right-from-square"></i> Cadastrar Assinatura no Meu Perfil
                </a>
            </div>
        ...
        <?php else: ?>
            ...
            <div class="form-card">
                <form method="POST">
                    <input type="hidden" name="edit_id_post" value="<?php echo $modo_edicao ? $edit_id : 0; ?>">
                    ...
                    <div class="form-section">
                        ...
                        <div class="grid-2">
                            ...
                            <div id="div_nome_anterior" style="display: <?php echo $c_proj_anterior == 'Sim' ? 'block' : 'none'; ?>;">
    ...
    <script>
        ...
        if (inputCat) {
            inputCat.addEventListener('focus', () => { listaCat.style.display = 'block'; });
