...
<head>
    ...
    <script>
        ...
        OneSignalDeferred.push(async function (OneSignal) {
            await OneSignal.init({
                ...
                safari_web_id: "web.onesignal.auto.sua_chave_safari_se_houver", // Opcional, o painel do OneSignal fornece
                notifyButton: {
                    enable: true, // Mostra um sininho no canto da tela para gerenciar permissões
                },
            });

            // A MÁGICA ACONTECE AQUI: Registra o ID do banco de dados no OneSignal
    ...
    <style>
        ...
        .alerta-box.aviso {
            ...
            border-right: 1px solid #faeccc;
            border-bottom: 1px solid #faeccc;
        }
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        <div class="sidebar-header">
            ...
            <button class="collapse-btn" id="collapse-btn" title="Minimizar Menu"><i
    ...
    <main class="main-content">
        <header class="header">
            ...
            <div class="user-info"
                ...
                <form method="GET"
                    style="margin:0; display:flex; align-items:center; gap:8px; background: #fff; padding: 6px 12px; border-radius: 6px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); border: 1px solid #eee;">
                    <i class="fa-solid fa-calendar-days" style="color: var(--fatec-red);"></i>
                    <select name="semestre" onchange="this.form.submit()"
                        style="border: none; background: transparent; font-weight: bold; color: #444; outline: none; cursor: pointer; font-size: 13px;">
                        <option value="Todos" <?php echo $filtro_semestre == 'Todos' ? 'selected' : ''; ?>>Histórico
                            Completo</option>
        ...
        <?php if ($funcao == 'Professor'): ?>
            <div class="dashboard-cards">
                <a href="meus_projetos.php?status_filtro=Aprovado" class="card" style="border-bottom-color: #3498db;">
                    <div class="card-icon" style="background: #e1f5fe; color: #0288d1;"><i
                            class="fa-solid fa-folder-open"></i></div>
                    <div class="card-info">
                        <h3>Projetos Ativos</h3>
                        <p><?php echo $kpi_projetos; ?></p>
                    </div>
                </a>
                ...
                <?php if ($kpi_projetos_devolvidos > 0): ?>
                    <a href="meus_projetos.php?status_filtro=Devolvido" class="card" style="border-bottom-color: #f39c12;">
                        <div class="card-icon" style="background: #fffdf5; color: #f39c12;"><i
                                class="fa-solid fa-rotate-left"></i></div>
                        <div class="card-info">
                            <h3>Devolvidos p/ Ajuste</h3>
                            <p style="color: #d68910;"><?php echo $kpi_projetos_devolvidos; ?></p>
                        </div>
                    </a>
                <?php endif; ?>

                <a href="meus_projetos.php" class="card" style="border-bottom-color: #27ae60;">
                    <div class="card-icon" style="background: #e8f5e9; color: #2e7d32;"><i
                            class="fa-solid fa-file-circle-check"></i></div>
                    <div class="card-info">
                        <h3>Relatórios Entregues</h3>
                        <p><?php echo $kpi_entregues; ?></p>
                    </div>
                </a>
                <a href="enviar_relatorio.php" class="card"
            ...
            <?php if (count($pendencias_professor) == 0): ?>
                <div class="alerta-box"
                    style="background: #f4fbf7; border-left: 5px solid #2ecc71; color: #27ae60; border: 1px solid #d1e7dd;">
                    <i class="fa-solid fa-circle-check"></i>
                    <div class="alerta-info">
                        <h4>Tudo em dia!</h4>
                        <p>Você não possui nenhum relatório pendente no semestre selecionado.</p>
                    </div>
                </div>
            <?php else: ?>
                <?php foreach ($pendencias_professor as $pendencia): ?>
                    <div class="alerta-box <?php echo $pendencia['status']; ?>">
                        <i
                            class="fa-solid <?php echo $pendencia['status'] == 'aviso' ? 'fa-clock' : 'fa-triangle-exclamation'; ?>"></i>
                        <div class="alerta-info">
                            ...
                            <a href="enviar_relatorio.php"
                                style="display:inline-block; margin-top:8px; color:inherit; font-weight:bold; text-decoration: none; border-bottom: 1px solid currentColor;">Ir
                                para envio <i class="fa-solid fa-arrow-right"></i></a>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
        ...
        <?php else: ?>
            ...
            <div class="dashboard-cards">
                ...
                <a href="analisar_solicitacoes.php?status_filtro=MeusAprovados" class="card"
                    style="border-bottom-color: #27ae60;">
                    <div class="card-icon" style="background: #e8f5e9; color: #2e7d32;"><i
                            class="fa-solid fa-thumbs-up"></i></div>
                    <div class="card-info">
                        <h3>Projetos Aprovei</h3>
                        <p><?php echo $kpi_meus_aprovados; ?></p>
                    </div>
                </a>
                <a href="analisar_solicitacoes.php?status_filtro=MeusDevolvidos" class="card"
                    style="border-bottom-color: #f39c12;">
                    <div class="card-icon" style="background: #fffdf5; color: #f39c12;"><i
                            class="fa-solid fa-rotate-left"></i></div>
                    <div class="card-info">
                        <h3>Projetos Devolvi</h3>
                        <p><?php echo $kpi_meus_devolvidos; ?></p>
                    </div>
                </a>
                <a href="analisar_solicitacoes.php?status_filtro=MeusRejeitados" class="card"
                    style="border-bottom-color: #c0392b;">
                    <div class="card-icon" style="background: #fdf2f2; color: #c0392b;"><i
                            class="fa-solid fa-thumbs-down"></i></div>
                    <div class="card-info">
                        <h3>Projetos Rejeitei</h3>
                        <p><?php echo $kpi_meus_rejeitados; ?></p>
                    </div>
                </a>
            </div>
            ...
            <?php if ($funcao == 'Diretor'): ?>
                <div class="card-orcamento">
                    ...
                    <div class="orcamento-box" style="background: <?php echo $kpi_saldo_hae < 0 ? '#fdf2f2' : '#f4fbf7'; ?>;">
                        <h3><i class="fa-solid fa-scale-unbalanced"
                                style="color: <?php echo $kpi_saldo_hae < 0 ? '#c0392b' : '#27ae60'; ?>;"></i> Saldo Restante
                        </h3>
            ...
            <div class="dashboard-cards">
                <a href="analisar_solicitacoes.php?status_filtro=Aprovados" class="card"
                    style="border-bottom-color: #3498db;">
                    <div class="card-icon" style="background: #e1f5fe; color: #0288d1;"><i
                            class="fa-solid fa-diagram-project"></i></div>
                    <div class="card-info">
                        <h3>Projetos Ativos (Geral)</h3>
                        <p><?php echo $kpi_projetos_ativos; ?></p>
                    </div>
                </a>

                <a href="acompanhar_relatorios.php" class="card" style="border-bottom-color: #16a085;">
                    <div class="card-icon" style="background: #e8f6f3; color: #16a085;"><i
                            class="fa-solid fa-calendar-check"></i></div>
                    <div class="card-info">
                        <h3>Relatórios (<?php echo substr($meses_nome[$mes_passado_num], 0, 3); ?>)</h3>
                        <p><?php echo $kpi_relatorios_mes; ?></p>
                    </div>
                </a>

                <?php if ($funcao == 'Diretor'): ?>
                    <a href="analisar_solicitacoes.php?status_filtro=Devolvidos" class="card"
                        style="border-bottom-color: #f39c12;">
                        <div class="card-icon" style="background: #fffdf5; color: #f39c12;"><i
                                class="fa-solid fa-rotate-left"></i></div>
                        <div class="card-info">
                            <h3>Devolvidos (Geral)</h3>
                            <p><?php echo $kpi_devolvidos_global; ?></p>
                        </div>
                    </a>

                    <a href="analisar_solicitacoes.php?status_filtro=Rejeitados" class="card"
                        style="border-bottom-color: #e74c3c;">
                        <div class="card-icon" style="background: #fdf2f2; color: #e74c3c;"><i class="fa-solid fa-ban"></i>
                        </div>
                        <div class="card-info">
                            <h3>Rejeitados (Geral)</h3>
                            <p><?php echo $kpi_rejeitados_global; ?></p>
                        </div>
                    </a>

                    <a href="listar_usuarios.php" class="card" style="border-bottom-color: #e67e22;">
                        <div class="card-icon" style="background: #fdf2e9; color: #e67e22;"><i class="fa-solid fa-users"></i>
                        </div>
                        <div class="card-info">
                            <h3>Total de Usuários</h3>
                            <p><?php echo $kpi_total_usuarios; ?></p>
                        </div>
                    </a>
                <?php endif; ?>
            </div>
            ...
            <?php if ($dia_atual >= 11): ?>
                <?php if (count($inadimplentes_geral) > 0): ?>
                    <div class="alerta-box atrasado">
                        ...
                        <div class="alerta-info">
                            <h4>Atenção: Relatório Atrasados Disponível</h4>
                            <p>Existem <strong><?php echo count($inadimplentes_geral); ?></strong> ocorrências de professores que
                                ultrapassaram o prazo de envio estipulado pela coordenação.</p>
                            <a href="relatorios_atrasados.php"
                                style="display:inline-block; margin-top:8px; color:inherit; font-weight:bold; text-decoration: none; border-bottom: 1px solid currentColor;">Gerar
                                Documento de Cobrança <i class="fa-solid fa-arrow-right"></i></a>
                        </div>
                    </div>
                <?php else: ?>
                    <div class="alerta-box"
            ...
            <?php if ($dia_atual <= 10): ?>
                <h2 style="font-size: 18px; color: #f39c12; margin-bottom: 15px;"><i class="fa-solid fa-bullhorn"></i> Período
                    de Cobrança Ativo (Dia 01 ao 10)</h2>
                <p style="color: #666; margin-bottom: 20px; font-size: 14px;">Lembre os professores de enviar os relatórios
                    referentes ao mês passado dentro do prazo legal.</p>

                <?php if (count($cobrancas_ativas_geral) > 0): ?>
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 15px;">
                        <?php foreach ($cobrancas_ativas_geral as $cob): ?>
                            <div
                                style="background: #fff; padding: 20px; border-radius: 8px; border-left: 4px solid #f39c12; box-shadow: 0 2px 5px rgba(0,0,0,0.05); border: 1px solid #eee;">
                                <h4 style="font-size: 15px; margin-bottom: 8px; color: #333;"><i class="fa-solid fa-user-tie"
                                        style="color:#ccc;"></i> <?php echo htmlspecialchars($cob['professor']); ?></h4>
                                <p style="font-size: 13px; color: #666; margin-bottom: 15px;">Falta entregar: <strong
                                        style="color: #d68910;"><?php echo $cob['mes_ano']; ?></strong><br><span
                                        style="font-size: 11px; color: #999;"><?php echo htmlspecialchars($cob['projeto']); ?></span>
                                </p>
                                ...
                                <?php if (strlen($num_whats) >= 10): ?>
                                    <a href="https://wa.me/55<?php echo $num_whats; ?>?text=<?php echo $msg; ?>" target="_blank"
                                        class="btn-whatsapp" style="background:#f39c12; width: 100%; justify-content: center;"><i
                                            class="fa-brands fa-whatsapp"></i> Enviar Lembrete</a>
                                <?php else: ?>
                                    <div
                                        style="font-size:12px; color:#888; background:#f4f4f4; padding: 8px; border-radius: 4px; text-align: center;">
                                        Sem Telefone</div>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                ...
                <?php else: ?>
                    <div class="alerta-box"
                        ...
                        <div class="alerta-info">
                            <h4>Excelente!</h4>
                            <p>Todos os professores já enviaram os relatórios referentes ao mês anterior.</p>
                        </div>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
