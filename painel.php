...
// AJUSTE CRUCIAL: Força o PHP a usar o fuso horário do Brasil, ignorando o relógio do servidor
...
// Inteligência de Data de Hoje (Agora com a hora e data certas do Brasil)
$hoje = new DateTime();
...
if ($funcao == 'Professor') {
    ...
    $stmt = $pdo->prepare("SELECT id, titulo_projeto, COALESCE(data_aprovacao_diretor, data_aprovacao_coordenador, data_criacao) AS data_base 
                           FROM solicitacoes_hae 
        ...
        while ($data_iteracao < $data_limite) {
            ...
            if (!$stmt_rel->fetch()) {
                ...
                if ($is_mes_anterior_imediato && $dia_atual <= 10) {
                    $pendencias_professor[] = [
                        'projeto' => $proj['titulo_projeto'],
                        'mes_ano' => $meses_nome[$mes_ref] . '/' . $ano_ref,
                        'status' => 'aviso',
                        'msg' => "Período de envio aberto. Você tem até o dia 10 para enviar o relatório de " . $meses_nome[$mes_ref] . "."
                    ];
                } else {
                    $pendencias_professor[] = [
                        'projeto' => $proj['titulo_projeto'],
                        'mes_ano' => $meses_nome[$mes_ref] . '/' . $ano_ref,
                        'status' => 'atrasado',
                        'msg' => "Prazo encerrado! Este relatório está oficialmente atrasado."
                    ];
                }
            }
...
// ==============================================================================================
// 2. LÓGICA DA DIREÇÃO / COORDENAÇÃO
// ==============================================================================================
else {
    ...
    $stmt_kpi2 = $pdo->query("SELECT COUNT(*) FROM solicitacoes_hae WHERE status_aprovacao = 'Aprovado'");
    $kpi_projetos_ativos = $stmt_kpi2->fetchColumn();
    ...
    $stmt = $pdo->query("SELECT s.id, s.titulo_projeto, COALESCE(s.data_aprovacao_diretor, s.data_aprovacao_coordenador, s.data_criacao) AS data_base, u.nome, u.telefone_whatsapp 
            ...
            if (!$stmt_rel->fetch()) {
                ...
                if ($is_mes_anterior_imediato && $dia_atual <= 10) {
                    $cobrancas_ativas_geral[] = [
                        ...
                        'projeto' => $proj['titulo_projeto'],
                        'mes_ano' => $meses_nome[$mes_ref] . '/' . $ano_ref,
                        'telefone' => $proj['telefone_whatsapp']
                    ];
                } else {
                    $inadimplentes_geral[] = [
                        'professor' => $proj['nome'],
...
<head>
    ...
    <style>
        ...
        .card { 
            background: #fff; padding: 25px; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.03); 
            display: flex; align-items: center; gap: 20px; transition: 0.3s; 
            border: 1px solid #eee; border-bottom: 4px solid var(--fatec-red);
        ...
        .tabela-inadimplentes { width: 100%; border-collapse: collapse; background: #fff; border-radius: 10px; overflow: hidden; box-shadow: 0 4px 10px rgba(0,0,0,0.03); border: 1px solid #eee; }
        .tabela-inadimplentes th { background: #fdf2f2; color: #c0392b; padding: 15px 20px; text-align: left; font-size: 13px; text-transform: uppercase; border-bottom: 2px solid #fad2d6;}
...
<body>
    ...
    <main class="main-content">
        <header class="header">
            ...
            <!-- DATA CORRIGIDA AQUI! O PHP vai obedecer o fuso do Brasil que configuramos -->
            <div class="user-info">Hoje é <strong><?php echo date('d/m/Y'); ?></strong></div>
        </header>
            ...
            <div class="dashboard-cards">
                ...
                <a href="enviar_relatorio.php" class="card" style="<?php echo count($pendencias_professor) > 0 ? 'border-bottom-color: #e74c3c;' : 'border-bottom-color: #2ecc71;'; ?>">
                    ...
                    <div class="card-info">
                        ...
                        <p style="color: <?php echo count($pendencias_professor) > 0 ? '#c0392b' : '#27ae60'; ?>;"><?php echo count($pendencias_professor); ?></p>
                    </div>
                </a>
            </div>
            ...
            <?php if (count($pendencias_professor) == 0): ?>
                <div class="alerta-box" style="background: #f4fbf7; border-left: 5px solid #2ecc71; color: #27ae60; border: 1px solid #d1e7dd;">
                    ...
                    <div class="alerta-info">
                        <h4>Tudo em dia!</h4>
                        <p>Você não possui nenhum relatório de HAE pendente no momento. Ótimo trabalho!</p>
                    </div>
                </div>
            <?php else: ?>
        ...
        <?php else: ?>
            <div class="dashboard-cards">
                <a href="analisar_solicitacoes.php" class="card" style="<?php echo $kpi_analises > 0 ? 'border-bottom-color: #f39c12;' : 'border-bottom-color: #2ecc71;'; ?>">
                    ...
                    <div class="card-info">
                        <h3>Aguardando sua Análise</h3>
                        <p style="color: <?php echo $kpi_analises > 0 ? '#d68910' : '#27ae60'; ?>;"><?php echo $kpi_analises; ?></p>
                    </div>
                </a>
                <a href="acompanhar_relatorios.php" class="card">
                    ...
                    <div class="card-info">
                        <h3>Relatórios Rec. (Mês <?php echo $mes_passado_num; ?>)</h3>
            ...
            <?php if ($dia_atual >= 11): ?>
                <h2 style="font-size: 18px; color: #c0392b; margin-bottom: 15px;"><i class="fa-solid fa-file-invoice"></i> Relatório de Inadimplência</h2>
                <p style="color: #666; margin-bottom: 20px; font-size: 14px;">Professores que ultrapassaram o prazo limite de envio dos relatórios mensais estipulado pela coordenação.</p>
                
                <?php if (count($inadimplentes_geral) > 0): ?>
                    <table class="tabela-inadimplentes" style="margin-bottom: 40px;">
                        <thead>
                            <tr>
                                <th style="width: 25%;">Professor(a)</th>
                                <th>Mês em Atraso</th>
                                <th style="width: 35%;">Projeto Vinculado</th>
                                <th style="text-align: right;">Ação</th>
                            </tr>
                        </thead>
                        <tbody>
                            <?php foreach ($inadimplentes_geral as $inadimplente): ?>
                                <tr>
                                    <td><strong><i class="fa-solid fa-user-tie" style="color:#aaa; margin-right:5px;"></i> <?php echo htmlspecialchars($inadimplente['professor']); ?></strong></td>
                                    <td><span style="background: #fee2e2; color: #b91c1c; padding: 4px 8px; border-radius: 4px; font-weight: bold; font-size: 12px;"><?php echo $inadimplente['mes_ano']; ?></span></td>
                                    <td><?php echo htmlspecialchars($inadimplente['projeto']); ?></td>
                                    <td style="text-align: right;">
                                        <?php 
                                            $num_whats = preg_replace('/[^0-9]/', '', $inadimplente['telefone']);
                                            $msg = urlencode("Olá professor(a)! O prazo para envio do relatório HAE de " . $inadimplente['mes_ano'] . " encerrou no dia 10. Consta em nosso sistema que o projeto '" . $inadimplente['projeto'] . "' está com o relatório pendente. Por favor, regularize a situação o mais rápido possível.");
                                        ?>
                                        <?php if(strlen($num_whats) >= 10): ?>
                                            <a href="https://wa.me/55<?php echo $num_whats; ?>?text=<?php echo $msg; ?>" target="_blank" class="btn-whatsapp">
                                                <i class="fa-brands fa-whatsapp" style="font-size: 14px;"></i> Cobrar URGENTE
                                            </a>
                                        <?php else: ?>
                                            <span style="font-size:12px; color:#888; background:#f4f4f4; padding: 6px 10px; border-radius: 4px;">Sem Telefone</span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                ...
                <?php else: ?>
                    <div class="alerta-box" style="background: #f4fbf7; border-left: 5px solid #2ecc71; color: #27ae60; margin-bottom: 40px; border: 1px solid #d1e7dd;">
                        <i class="fa-solid fa-circle-check"></i>
                        <div class="alerta-info">
                            <h4>Nenhum inadimplente!</h4>
                            <p>Todos os professores enviaram os relatórios obrigatórios com sucesso.</p>
                        </div>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
            ...
            <?php if ($dia_atual <= 10): ?>
                ...
                <p style="color: #666; margin-bottom: 20px; font-size: 14px;">Estes relatórios referentes ao mês passado ainda não foram enviados, mas estão dentro do prazo legal.</p>
                
                <?php if (count($cobrancas_ativas_geral) > 0): ?>
                    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(320px, 1fr)); gap: 15px;">
                        <?php foreach ($cobrancas_ativas_geral as $cob): ?>
                            <div style="background: #fff; padding: 20px; border-radius: 8px; border-left: 4px solid #f39c12; box-shadow: 0 2px 5px rgba(0,0,0,0.05); border-top: 1px solid #eee; border-right: 1px solid #eee; border-bottom: 1px solid #eee;">
                                ...
                                ?>
                                <?php if(strlen($num_whats) >= 10): ?>
                                    <a href="https://wa.me/55<?php echo $num_whats; ?>?text=<?php echo $msg; ?>" target="_blank" class="btn-whatsapp" style="background:#f39c12; width: 100%; justify-content: center;">
                                        <i class="fa-brands fa-whatsapp" style="font-size: 14px;"></i> Enviar Lembrete
                                    </a>
                                <?php else: ?>
                                     <div style="font-size:12px; color:#888; background:#f4f4f4; padding: 8px; border-radius: 4px; text-align: center;">Telefone não cadastrado</div>
                                <?php endif; ?>
                            </div>
                        <?php endforeach; ?>
                    </div>
                <?php else: ?>
                    <div class="alerta-box" style="background: #f4fbf7; border-left: 5px solid #2ecc71; color: #27ae60; border: 1px solid #d1e7dd;">
                        ...
                        <div class="alerta-info">
                            <h4>Excelente!</h4>
                            <p>Todos os professores já se adiantaram e enviaram os relatórios referentes ao mês anterior.</p>
                        </div>
                    </div>
                <?php endif; ?>
            <?php endif; ?>
